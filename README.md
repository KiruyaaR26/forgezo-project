...
### Lokasi folder `custom` di server (Arch Linux)
...
```
mkdir /var/lib/forgejo/custom
```


## untuk css
```bash
mkdir /var/lib/forgejo/costume/public/
```

```bash
mkdir /var/lib/forgejo/custom/public/assets/css/<nama file>.css
```
...
```bash
sudo nvim /var/lib/forgejo/custom/public/assets/css/<nama file>.css
```

> lalu isi dengan file css yang ada digithub

Paste/edit isinya, save, lalu:
...
```bash
sudo mkdir -p /var/lib/forgejo/custom/templates/<folder-sesuai-source>
```

### atau

```bash
sudo mkdir -p /var/lib/forgejo/custom/templates/<file>.tmpl
```

> ini menyesuaikan dengan yang ada di github

```bash
sudo tee /var/lib/forgejo/custom/templates/<folder>/<file>.tmpl > /dev/null <<'EOF'
[isi template]
EOF
```
